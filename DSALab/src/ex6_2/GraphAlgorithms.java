...
public class GraphAlgorithms
{
   ...
   {  if (!graph.containsVertex(startVertex))
    ...
    /**
     * -------------------------------my function-------------------------------
     * This is the entry point of recursive depthFirstSearch
     */
    public static <E> List<Vertex<E>> recursiveDepthFirstSearch (GraphADT<E> graph, Vertex<E> startVertex) {
        if (!graph.containsVertex(startVertex)) {
            throw new IllegalArgumentException();
        }
        List<Vertex<E>> visitedList = new LinkedList<Vertex<E>>();
        
        return innerRecursiveDepthFirstSearch(startVertex, visitedList);
    }
    
    /**
     * -------------------------------my function------------------------------- 
     * This is inner recursive depthFirstSearch 
     */
    private static <E> List<Vertex<E>> innerRecursiveDepthFirstSearch (Vertex<E> processVertex, List<Vertex<E>> visitedList) {
        visitedList.add(processVertex);
        Iterator<Vertex<E>> iterator = processVertex.adjacentVertices().iterator();
        while (iterator.hasNext()) {
            Vertex<E> nextVertex = iterator.next();
            if (!visitedList.contains(nextVertex)) {
                innerRecursiveDepthFirstSearch(nextVertex, visitedList);
            }
        }
        return visitedList;
    }
   ...
   {  GraphADT<String> graph = new AdjacencyListGraph<String>();
      ...
      //This is my chanl of code to test recursive depth-first search.
      System.out.println("Performing Recursive Depth-first search from Auc:");
      List<Vertex<String>> recurList = recursiveDepthFirstSearch(graph,auckland);
      System.out.println(recurList);
       
      
       System.out.println("-----------------another test case------------------");
      System.out.println("Performing Depth-first search from Fij:");
      list = depthFirstSearch(graph, fiji);
      System.out.println(list);
      
      System.out.println("Performing Recursive Depth-first search from Fij:");
      recurList = recursiveDepthFirstSearch(graph,fiji);
      System.out.println(recurList);
   }
}
